...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    // NO iniciar polling aquí — se activa solo al enviar un mensaje
  }
  ...
  abrirConversacion(id: number) {
    ...
    this.convService.obtenerConversacion(id).subscribe({
      next: data => {
        ...
        // Restaurar tipoContenido AUDIO_TTS para mensajes con audio guardado en BD
  ...
  private enviarMensajeAConversacion(convId: number, texto: string, tipo: 'TEXTO' | 'IMAGEN' | 'VIDEO') {
    ...
    this.debeScroll = true;
    this.esperando = true;
    this.esperandoMejor = true;        // activar aquí: esperar al juez tras respuesta de IAs
    this.esperandoMejorTicks = 30;     // 30 ticks × 2s = 60s máx esperando mejor respuesta
    this.iniciarPolling(); // Reactivar polling si fue detenido
    this.cdr.detectChanges();
  ...
  enviarTTS() {
    ...
    const doEnvio = (convId: number) => {
      ...
      // PASO 1: guardar el mensaje en el backend y obtener el ID real
      this.mensajeService.enviarMensaje(convId, texto, 'AUDIO_TTS').toPromise()
        .then((mensajeGuardado: any) => {
          ...
          // Actualizar el ID del optimista al real
          ...
          // PASO 2: llamar a TTS pasando el mensajeId para que el backend persista el audio
          return this.http.post(`${API}/media/tts`, { texto, mensajeId }, { responseType: 'blob' })
            ...
            .then((blob: any) => {
              ...
              // Mostrar el audio inmediatamente usando blob URL en memoria
  ...
  iniciarPolling() {
    // Solo crear un nuevo intervalo si no hay uno activo ya
    ...
    this.pollSub = interval(2000).subscribe(() => {
      ...
      if (this.esperandoMejor) {
        ...
        if (this.esperandoMejorTicks <= 0) {
          this.esperandoMejor = false;
          // Solo detener si no hay nada más pendiente
  ...
  recargar() {
    ...
    this.convService.obtenerConversacion(this.conversacionActualId).subscribe({
      next: data => {
        ...
        const nuevos: Mensaje[] = data.mensajes || [];
        // Compute mejorCambio BEFORE overwriting this.mensajes
        ...
        if (usuariosNuevos >= usuariosActuales) {
          // Audio ahora persiste en DB como data:audio/wav;base64 en urlArchivo.
          // Solo necesitamos restaurar el tipoContenido AUDIO_TTS que el backend
          // a veces devuelve correctamente, pero por si acaso lo forzamos por modeloIA.
          this.mensajes = nuevos.map(m => {
            const tieneAudio = (m.respuestasIA || []).some((r: any) => r.modeloIA === 'GROQ_TTS_ORPHEUS');
            if (tieneAudio && m.tipoContenido !== 'AUDIO_TTS') {
              return { ...m, tipoContenido: 'AUDIO_TTS' as any };
  ...
  verificarPendientes() {
    ...
    for (const m of this.mensajes) {
      ...
      // Si el mensaje ya tiene una tarjeta sintética de audio, no está pendiente
    ...
    if (this.pendientes.size === 0) {
      this.esperando = false;
      // esperandoMejor se activa SOLO desde enviarMensaje(), no aquí.
      // Si ya terminó, detener el polling para no seguir haciendo requests.
      if (!this.esperandoMejor) {
        this.pollSub?.unsubscribe();
      }
    }
  ...
  getIAsEsperadas(tipo: string): string[] {
    switch (tipo) {
      ...
      case 'AUDIO_TTS': return [];   // el audio lo genera el front, no hay IAs pendientes
...
getProxiedUrl(url: string | null | undefined): string {
    ...
    if (url.includes('openrouter.ai')) {
        if (url.includes('/videos/')) {
            result = `https://gpcueb.org/siftlyai/api/auth/public/video?url=${encodeURIComponent(url)}`;
        } else if (url.includes('/image') || url.includes('.png') || url.includes('.jpg')) {
            result = `https://gpcueb.org/siftlyai/api/auth/public/image?url=${encodeURIComponent(url)}`;
        }
  ...
  isAudioUrl(resp?: any): boolean {
    ...
    return l.startsWith('data:audio') || l.endsWith('.wav') || l.endsWith('.mp3') ||
           l.endsWith('.ogg') || l.endsWith('.opus') || l.includes('audio') || url.startsWith('blob:');
  }

  isImagenUrl(url?: string): boolean {
    if (!url) return false;
    const l = url.toLowerCase();
  ...
  isVideoUrl(url?: string): boolean {
    ...
    if (l.includes('openrouter.ai') && l.includes('/videos/')) {
      return true;
    }
    return l.endsWith('.mp4') || l.endsWith('.webm') || l.endsWith('.mov') || l.includes('video');
  }
